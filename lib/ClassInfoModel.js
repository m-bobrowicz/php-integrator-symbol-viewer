...
var parseOwnSymbols = (symbols, onlyStatic = false) => _(symbols)
  .values()
  .filter((symbol) => {
    if(onlyStatic) {
  ...
  .map(s => _.extend(s, {
      declaringStructureKey: s.declaringStructure.type.toUpperCase()[0] + '-' + s.declaringStructure.name
  }))
  .sortBy('name')
  .value();

var parseDependencies = ({constants, properties, methods, name}) =>
    _([constants, properties, methods])
        .flatMap(_.values)
        .filter(s => s.declaringStructure.name !== name)
        .groupBy(s => ''
            + s.declaringStructure.type.toUpperCase()[0]
            + '-'
            + s.declaringStructure.name
        )
        .keys()
        .map(d => d.split('-'))
        .map(([bulletChar, name]) => ({
        ...
        .sortBy(({bulletChar, shortName}) => bulletChar + shortName)
        .value();
